...
pub fn gpu(config: Config) -> ocl::Result<()> {
    ...
    loop {
        ...
        loop {
            ...
            let work_start_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();

            // if we know the previous work duration, sleep for 95% of the duration, to conserve CPU
            if work_duration_millis != 0 {
                std::thread::sleep(std::time::Duration::from_millis(
                    work_duration_millis * 950 / 1000,
                ));
            }
            ...
            let work_end_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
            work_duration_millis = (work_end_time.as_secs() * 1000
                + work_end_time.subsec_nanos() as u64 / 1000000)
                - (work_start_time.as_secs() * 1000
                    + work_start_time.subsec_nanos() as u64 / 1000000);
